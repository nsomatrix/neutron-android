...
import com.nsomatrix.neutron.config.ProfilesManager;

import static com.nsomatrix.neutron.util.Constants.PREF_DEFAULT_PROFILE;
...
public class MigrationUtils {
	...
	private static void moveConfigs(Context context) {
		...
		for (File srcConf : srcConfDir.listFiles()) {
			...
			if (fileName.equals("com.nsomatrix.neutron_preferences")) {
				continue;
			}
	...
	private static void moveDatabase(Context context) {
		File oldDb = new File(Config.getEmulatorDir(), "J2ME-apps.db");
		File dstDb = new File(Config.getEmulatorDir(), "neutron-apps.db");
		if (oldDb.exists() && !dstDb.exists()) {
			oldDb.renameTo(dstDb);
			new File(oldDb + "-shm").renameTo(new File(dstDb + "-shm"));
			new File(oldDb + "-wal").renameTo(new File(dstDb + "-wal"));
		}
